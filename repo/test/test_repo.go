...
func NewTestRepo() (mr repo.Repo, err error) {
	...
	for _, k := range datasets {
		...
		dskey, err = mr.CreateDataset(tc.Name, tc.Input, datafile, true)
		if err != nil {
...
func NewMemRepoFromDir(path string) (repo.Repo, crypto.PrivKey, error) {
	pro, pk, err := ReadRepoConfg(filepath.Join(path, "config.yaml"))
	if err != nil {
		return nil, pk, err
	}

	ms := cafs.NewMapstore()
	mr, err := repo.NewMemRepo(pro, ms, repo.MemPeers{}, &analytics.Memstore{})
	if err != nil {
		return mr, pk, err
	}
	mr.SetPrivateKey(pk)
...
// ReadRepoConfig reads configuration data from a file
func ReadRepoConfg(path string) (pro *profile.Profile, pk crypto.PrivKey, err error) {
